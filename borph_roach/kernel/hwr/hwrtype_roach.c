...
static ssize_t roach_recv_iobuf (struct hwr_iobuf* iobuf)
{
	...
	*/
	...
	for (i = 0; i < iobuf->size; i +=2)
	{
		*dst = FPGA_READW(iobuf->location + i + iobuf->offset);
		dst++; 
	}
...
static int __init hwrtype_roach_init(void)
{
	...
	roach_ebc->cpld_virt = ioremap64(ROACH_CPLD_BASE,ROACH_CPLD_LENGTH);

	PDEBUG(9, "new I/O memory range 0x%08x to 0x%08x allocated (virt:0x%p)\n",
		       (resource_size_t) roach_ebc->cpld_base,
		       (resource_size_t) (roach_ebc->cpld_base + ROACH_CPLD_LENGTH),
		       roach_ebc->cpld_virt);
	...
	if (!request_mem_region(ROACH_SMAP_BASE, ROACH_SMAP_LENGTH, "roach-selectmap")) {
		printk(KERN_ERR "%s: memory range 0x%08x to 0x%08x is in use\n",
			        "roach-selectmap",
			        (resource_size_t) ROACH_SMAP_BASE,
			        (resource_size_t)(ROACH_SMAP_BASE + ROACH_SMAP_LENGTH));
		goto out_freemem;
	}

	roach_ebc->smap_base = ROACH_SMAP_BASE;
	roach_ebc->smap_virt = ioremap64(ROACH_SMAP_BASE,ROACH_SMAP_LENGTH);

	PDEBUG(9, "new I/O memory range 0x%08x to 0x%08x allocated (virt:0x%p)\n",
		       (resource_size_t) roach_ebc->smap_base,
		       (resource_size_t) (roach_ebc->smap_base + ROACH_SMAP_LENGTH),
		       roach_ebc->smap_virt);
	...
	if (!request_mem_region(ROACH_EBC_BASE, ROACH_EBC_LENGTH, "roach-ebc")) {
		printk(KERN_ERR "%s: memory range 0x%08x to 0x%08x is in use\n",
			        "roach-fpga",
			        (resource_size_t) ROACH_EBC_BASE,
			        (resource_size_t)(ROACH_EBC_BASE + ROACH_EBC_LENGTH));
		goto out_freemem;
	}

	roach_ebc->ebc_base = ROACH_EBC_BASE;
	roach_ebc->ebc_virt = ioremap64(ROACH_EBC_BASE,ROACH_EBC_LENGTH);

	PDEBUG(9, "new I/O memory range 0x%08x to 0x%08x allocated (virt:0x%p)\n",
		       (resource_size_t) roach_ebc->ebc_base,
		       (resource_size_t) (roach_ebc->ebc_base + ROACH_EBC_LENGTH),
		       roach_ebc->ebc_virt);
