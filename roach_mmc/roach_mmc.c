...
//Manually advance the MMC CLK
void rmmc_man_adv(struct rmmc_host *host)
{
...
//Set MMC Write DAT
void rmmc_set_dat(struct rmmc_host *host, unsigned char data)
{
  out_8(host->base + RMMC_REG_DAT, data);
}

//Set MMC Write CMD
void rmmc_set_cmd(struct rmmc_host *host, unsigned char data)
{
  out_8(host->base + RMMC_REG_CMD, data);
}

//Get MMC CMD
unsigned char rmmc_get_cmd(struct rmmc_host *host)
{
  return in_8(host->base + RMMC_REG_CMD);
}

//Get MMC Read DAT 
unsigned char rmmc_get_dat(struct rmmc_host *host)
{
...
static inline char *rmmc_sg_to_buffer(struct scatterlist *sg)
{
  return sg_virt(sg);
}
...
int rmmc_send_cmd_byte(struct rmmc_host *host, unsigned char data)
{
  unsigned char i = 0;
...
void rmmc_tick_clock(struct rmmc_host *host, unsigned int cycles)
{
...
int rmmc_get_cmd_byte(struct rmmc_host *host)
{
  unsigned char i = 0;
  unsigned char din = 0;
...
int rmmc_rdauto_wait(struct rmmc_host *host, int max_ticks)
{
  ...
  while (1){
    ...
    if (n == max_ticks){
      printk(KERN_ERR "rmmc: failed after %d tries\n", n);
      return -ETIMEDOUT;
    }
    n++;
  }
...
int rmmc_read_data(struct rmmc_host *host, struct mmc_data *data)
{
...
#ifdef RMMC_DEBUG
  printk(KERN_INFO "rmmc: blocksize = %d, #blocks = %d, noterm = %d\n", data->blksz, data->blocks, !(data->stop));
#endif
  ...
  /* TODO: must ensure blocksize of 512 */
    ...
    for (i = 0; i < (data->blksz - partial); i++){
      ...
      if (i == 0 && partial == 0) {
        if (poll) {
          ...
          if ((ret = rmmc_rdauto_wait(host, RMMC_DATA_WAIT_MAX))){
            goto done;
          }
        }
      }
  ...
  if (data->stop) {
#ifdef RMMC_DEBUG
    printk(KERN_INFO "rmmc: sending stop command\n");
#endif
    rmmc_send_command(host, data->stop);
  } else {
    rmmc_tick_clock(host, 4); /* This needs to be high for an unknown reason */
  }
  return ret;
}

int rmmc_send_data_byte(struct rmmc_host *host, unsigned char data, int ticks, int poll)
{
  unsigned char i = 0;
...
int rmmc_send_crc(struct rmmc_host *host, unsigned char *data, int blocksz, int ticks, int poll)
{
  ...
  switch(ticks){
    ...
    case 8:/*1-bit mode*/
#ifdef RMMC_DEBUG
      printk(KERN_INFO "rmmc_send_crc:1-bit mode\n");
#endif
...
int rmmc_write_data(struct rmmc_host *host, struct mmc_data *data)
{
...
#ifdef RMMC_DEBUG
  printk(KERN_INFO "\nrmmc: blocksize = %d, #blocks = %d, noterm = %d\n", data->blksz, data->blocks, !(data->stop));
#endif
  ...
  while (1){
    ...
    if(RMMC_CRC_OK(din)){
      //printk("rmmc_debug: CRC ok!\n");
      data->bytes_xfered += data->blksz;
      din = rmmc_get_data_byte(host, 1, 1);
      if (!RMMC_WR_RDY(din)){
        //printk("rmmc_debug: wr busy\n");
        break;
      }
    } else{
...
irqreturn_t rmmc_irq(int irq, void *dev_id)
{
  ...
  unsigned char src = in_8((void*)(host->base + RMMC_REG_IRQ_REG));
...
#ifdef RMMC_DEBUG
  printk(KERN_INFO "rmmc-wrtest: ioremap - req = %x, ret = %x\n", ROACH_CPLD_BASE, (unsigned int) host->base);
#endif
...
static void rmmc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
  ...
  if (cmd->data && !cmd->error){
    if (cmd->data->flags & MMC_DATA_READ){
      ...
      cmd->error = rmmc_write_data(host, cmd->data);
      goto done;
    } else {
      cmd->error = -ETIMEDOUT;
      goto done;
...
static void rmmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
  ...
  spin_unlock_bh(&host->lock);
  printk(KERN_INFO "rmmc: set ios, clk = %x, dw = %x\n", host->clk, host->bus_width);
}
...
static int __devinit rmmc_init(struct device *dev)
{
  ...
  out_8((void*)(host->base + RMMC_REG_IRQ_MASK), CARD_IRQ_EN);
...
static int __init rmmc_drv_init(void)
{
  ...
  printk(KERN_INFO DRIVER_NAME ": ROACH MMC driver\n");
