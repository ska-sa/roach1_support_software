...
static volatile int collect_child;
...
static void release_clone(struct katcp_dispatch *d)
{
  ...
  if(d->d_clone < s->s_used){
    ...
    /* exchange entries to keep used ones contiguous */
    s->s_clients[i] = dt;
    s->s_clients[it] = d;
...
{
  int run, i, nfd, fd, max, result, suspend, status, status_child;
  unsigned int len;
  ...
  /* current and previous signal states*/
  struct sigaction ac, ap;
  sigset_t mc, mp;
  ...
  sigprocmask(SIG_SETMASK, NULL, &mc);
  sigaddset(&mc, SIGCHLD);
  sigprocmask(SIG_SETMASK, &mc, &mp);
  sigdelset(&mc, SIGCHLD);

  ac.sa_handler = handle_child;
  ac.sa_flags = 0;
  sigemptyset(&(ac.sa_mask));
  sigaction(SIGCHLD, &ac, &ap);

  /* WARNING: don't exit out of this function without cleaning up signals */
  ...
  while(run){
    ...
    result = pselect(max + 1, &fsr, &fsw, NULL, suspend ? NULL : &delta, &mc);
#ifdef DEBUG
    ...
    if(collect_child){
      while((pid = waitpid(WAIT_ANY, &status_child, WNOHANG)) > 0){
        reap_shared_katcp(dl, pid, status_child);
      }
      collect_child = 0;
    }
  ...
  /* WARNING: ensure that we reset signal handlers */
  sigaction(SIGCHLD, &ap, NULL);
  sigprocmask(SIG_BLOCK, &mp, NULL);
