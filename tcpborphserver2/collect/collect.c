...
struct collect_state *create_collect(char *binary, char *text, char *meta, int port, int verbose)
{
  ...
  if(meta){
    if(text && (strcmp(meta, text) == 0)){
      cs->c_meta = cs->c_text;
    } else {
      if(strcmp(meta, "-")){
        cs->c_meta = fopen(meta, "w");
        if(cs->c_meta == NULL){
          fprintf(stderr, "create: unable to open %s for writing\n", meta);
          destroy_collect(cs);
          return NULL;
...
int main(int argc, char **argv)
{
    ...
    if (argv[i][0] == '-') {
      ...
      switch (c) {
        ...
        case 'v':
          ...
          break;
        case 'h' :
          fprintf(stderr, "usage: %s [-o binary-output] [-d decoded-output] [-m header-information] [-p receive-port]\n", argv[0]);
          return 0;
          break;
        case 'm' :
          j++;
  ...
  cs = create_collect(binary, text, meta, port, verbose);
  if(cs == NULL){
