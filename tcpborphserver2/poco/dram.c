...
int run_dram_poco(struct katcp_dispatch *d, void *data)
{
  ...
  uint32_t status, control, stamp, value, delta;
  ...
  if(r->r_last > 0){
    delta = stamp - r->r_last;
    if(delta > r->r_delta){
      if(r->r_delta > 0){
        log_message_katcp(d, KATCP_LEVEL_WARN, NULL, "packet loss before count %u", stamp);
        r->r_gap++;
      } else {
        r->r_delta = delta;
      }
    } else if(delta < r->r_delta){
      if(delta == 0){
        log_message_katcp(d, KATCP_LEVEL_WARN, NULL, "timestamp has not yet ticked over despite flag being ready");
        r->r_nul++;
      } else {
        r->r_gap++;
        r->r_delta = delta;
    ...
    log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "timestamp delta is %u", delta);
  } 
  r->r_last = stamp;
  ...
  if(!(status & IS_DONE_VACC_STATUS0)){ /* nothing ready yet, and errors */
  ...
  rr = read_pce(d, r->r_stamp, &stamp, 0, 4);
  if(rr != 4){
    log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "unable to reread timestamp for dram %d", r->r_number);
    return -1;
  }

  if(stamp != r->r_last){
    log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "timestamp advanced from %d to %d while dumping data", r->r_last, stamp);
    r->r_corrupt++;
    return -1;
  }
...
int toggle_dram_poco(struct katcp_dispatch *d, void *data, int start)
{
  ...
  if(start == 0){ /* stop */
    ...
    log_message_katcp(d, KATCP_LEVEL_INFO, NULL, "encountered %u timestamp gaps", r->r_gap);
    r->r_gap = 0;

    log_message_katcp(d, KATCP_LEVEL_INFO, NULL, "noted %u corrupt dram blocks", r->r_corrupt);
    r->r_corrupt = 0;
    ...
    log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "expected delta is %u", r->r_delta);
  ...
  value = (2 * sp->p_accumulation_length) / 5;
