...
{
  /* logic:  */
  /* poke auto means that we are being run from a timer, advance our state */
  /* machine, by looking at state value (initial value zero, but never changed */
  /* by subsequent api logic) */

  /* poke start and stop are requests from the user: with c_start, c_stop and c_prep set, */
  /* requests from user can warp state machine, either from idle/prep to prep or influence timers */
  ...
  switch(poke){
    ...
    case CAPTURE_POKE_AUTO : /* automatic state machine runner */
      switch(cp->c_state){
        ...
        case IDLE_GENERIC_CAPTURE :
          log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "running capture timer while idle is unexpected");
          return -1;
    ...
    case CAPTURE_POKE_START : /* user notification start */
      ...
      switch(cp->c_state){
        ...
        default :
          ...
          return -1; /* won't start if we have already started */
      }
    ...
    case CAPTURE_POKE_STOP  : /* user notification - stop */
      if(cp->c_stop.tv_sec == 0){
        log_message_katcp(d, KATCP_LEVEL_FATAL, NULL, "having no stop time set when requesting stop is a major logic failure");
      }
      ...
      switch(cp->c_state){
        case IDLE_GENERIC_CAPTURE :
          ...
          log_message_katcp(d, KATCP_LEVEL_WARN, NULL, "setting a stop for %s at %lu.%lus while idle", cp->c_name, cp->c_stop.tv_sec, cp->c_stop.tv_usec);
          break;
