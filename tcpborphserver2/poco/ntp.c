...
int recv_ntp_poco(struct katcp_dispatch *d, struct ntp_sensor_poco *nt)
{
  ...
  switch(field){
    ...
    case NTP_LEAPI_FIFTYNINE :
      break;
    default :
      log_message_katcp(d, KATCP_LEVEL_WARN, NULL, "ntp reports error status with leap indicator 0x%x and full word 0x%x", field, nm->n_status);
      return -1;
  }

  field = GET_BITS(nm->n_status, NTP_CLOCKSRC_SHIFT, NTP_CLOCKSRC_MASK);
  if(field != NTP_CLOCKSRC_NTPUDP){
    log_message_katcp(d, KATCP_LEVEL_WARN, NULL, "ntp synchronised by unusual means %d in status word 0x%x", field, nm->n_status);
  }
    ...
    if((field & (NTP_PEER_STATUS_CONFIGURED | NTP_PEER_STATUS_REACHABLE)) == (NTP_PEER_STATUS_CONFIGURED | NTP_PEER_STATUS_REACHABLE)){
      ...
      switch(field){
        ...
        case NTP_PEER_SELECT_CLOSE : 
          ...
          break;
        default :
          log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "ntp peer %d not yet ready with code %d", ntohs(np->p_id), field);
          break;
      }

    } else {
      log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "ntp peer %d either unreachable or unconfigured", ntohs(np->p_id));
    }
  }

  if(good > nt->n_sync){
    if(check_time_poco(d, 0) < 0){
      log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "detected time warp or other timing problem");
