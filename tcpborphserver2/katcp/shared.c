...
int enter_mode_katcp(struct katcp_dispatch *d, unsigned int mode, char *flags)
{
  ...
  if(uname(&u) == 0){
    len = strlen(u.nodename) + 1;
    if(s->s_vector[s->s_mode].e_name){
      len += strlen(s->s_vector[s->s_mode].e_name) + 1;
    }

    tmp = realloc(s->s_default_module, sizeof(char) * len);
    if(tmp != NULL){
      s->s_default_module = tmp;
      if(s->s_vector[s->s_mode].e_name){
        snprintf(s->s_default_module, len, "%s.%s", u.nodename, s->s_vector[s->s_mode].e_name);
      } else {
        snprintf(s->s_default_module, len, "%s", u.nodename);
      }
      s->s_default_module[len - 1] = '\0';
    }
  }
