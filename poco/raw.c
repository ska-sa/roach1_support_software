...
int read_cmd(struct katcp_dispatch *d, int argc)
{
  ...
  if(rr == length){
    ...
    append_buffer_katcp(d, KATCP_FLAG_LAST | KATCP_FLAG_BUFFER, data, length);
...
int write_cmd(struct katcp_dispatch *d, int argc)
{
  ...
  log_message_katcp(d, KATCP_LEVEL_DEBUG, NULL, "writing %lu bytes to %s at %u", length, name, start);
...
int setup_raw_poco(struct katcp_dispatch *d, char *bofs, int check)
{
  ...
  result += register_mode_katcp(d, "?listdev",    "lists available registers (?listdev)", &listdev_cmd, POCO_RAW_MODE);
  ...
  result += register_mode_katcp(d, "?tap-start", "start a tgtap instance (?tap-start register-name ip-address [port [mac]])", &start_tgtap_cmd, POCO_RAW_MODE);
  result += register_mode_katcp(d, "?tap-stop", "stop a tgtap instance (?tap-stop register-name)", &stop_tgtap_cmd, POCO_RAW_MODE);

#ifdef DEBUG
  result += register_mode_katcp(d, "?tick", "tests the scheduler (?tick milli-second-interval count)", &tick_cmd, POCO_RAW_MODE);
#endif
