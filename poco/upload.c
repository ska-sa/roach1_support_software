...
void upload_collect(struct katcp_dispatch *d, int status)
{
  ...
  if(WIFEXITED(status)){
    ...
    if(code > 0){
      log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "upload process exited with code %d", code);
    } else {
      log_message_katcp(d, KATCP_LEVEL_INFO, NULL, "upload process completed");
    }
  } else {
    log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "upload process failed");
  }
}
