...
void parse_device(char *string)
{
  ...
  device = temp_device;
  temp_device = strtok(NULL, ":");
  if(temp_device == NULL){
    return;
  }

  temp_rate = atoi(temp_device);
  if(temp_rate > 0){
    baud_rate = temp_rate;
  }
}
...
int main(int argc, char *argv[]){
  ...
  for(i=0; i < max_cmd; i++){
    ...
    if(strchr(msgs[i], '?') != NULL){
      ...
      timeout.tv_usec = 500000; /* plus half a second (500000 microseconds) */
...
static char *error_messages[MESSAGE_COUNT] = { 
  "Query error detected", 
  "Device-dependent error detected", 
  "Execution error detected", 
  "Command error detected", 
  "Device is switched over to manual control: user pressed LOCAL key", 
  "Device is switched on: user pressed the power button"
};

int error_exist(int status_register){
  int x;

  for(x = 0; x < MESSAGE_COUNT; x++){
    if((status_register >> (x + 2)) & 1){
      printf("%s\n", error_messages[x]);
    }
  }

  return (status_register & 0x3c) ? 1 : 0;
}
