...
static int physmap_flash_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < dev->num_resources; i++) {
		...
		printk(KERN_NOTICE "physmap: mapped phy (%d bits) %.8llx to virtual %.8lx\n", sizeof(phys_addr_t) * 8, (unsigned long long)dev->resource[i].start, (unsigned long)info->map[i].virt);
	...
	printk(KERN_NOTICE "physmap: about to add mtd devices\n");
...
/* need to wrap U64_C because concatenation doesn't dereference its args */
...
static struct resource physmap_flash_resource = {
	.start		= RESOURCE_SUFFIX(CONFIG_MTD_PHYSMAP_START),
	.end		  = RESOURCE_SUFFIX(CONFIG_MTD_PHYSMAP_START) + CONFIG_MTD_PHYSMAP_LEN - 1,
