...
#ifdef DEBUG
    default :
      fprintf(stderr, "major logic problem: event integer check not run on integer\n");
      abort();
      break;
#endif
  }
  ...
  if(cmp_time_katcp(&now, &legal) < 0){
    ...
    switch(a->a_type){
      ...
      case KATCP_SENSOR_BOOLEAN :
        ...
        log_message_katcp(d, KATCP_LEVEL_TRACE, NULL, "got integer result %d", ia->ia_current);
        break;
      default :
...
#ifdef DEBUG
    if(sn == NULL){
      fprintf(stderr, "run: logic problem: null sensor fields\n");
      abort();
    }
