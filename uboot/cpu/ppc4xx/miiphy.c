...
int phy_setup_aneg (char *devname, unsigned char addr)
{
	...
	miiphy_read (devname, addr, PHY_ANAR, &adv);
        /* there were extra fields in here, including fault trigger (!?) */
	adv |= (PHY_ANLPAR_TXFD | PHY_ANLPAR_TX | PHY_ANLPAR_10FD | PHY_ANLPAR_10);
#if defined(CONFIG_DP83865_PHY)
        adv &= ~(PHY_ANLPAR_NP);
        adv |= PHY_ANLPAR_PSB_802_3;
#else
        adv |= PHY_ANLPAR_T4;
#endif
        debug("miiphy: writing 0x%04x to ANAR\n", (unsigned int)adv);
	miiphy_write (devname, addr, PHY_ANAR, adv);

	miiphy_read (devname, addr, PHY_1000BTCR, &adv);
	adv |= (PHY_1000BTCR_1000FD | PHY_1000BTCR_1000HD);
#if defined(CONFIG_ROACH)
        adv &= ~(PHY_1000BTCR_MULTI);
#endif
        debug("miiphy: writing 0x%04x to BTCR\n", (unsigned int)adv);
	...
	miiphy_write (devname, addr, PHY_BMCR, ctl);
        ...
        debug("miiphy: triggering autonegotiation on phy 0x%x with 0x%04x\n", (unsigned int)addr, (unsigned int)ctl);
