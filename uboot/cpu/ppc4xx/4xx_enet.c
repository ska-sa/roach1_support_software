...
/* force loopback with packet rejection */
#undef FORCE_LOOPBACK 
...
static void ppc_4xx_eth_halt(struct eth_device *dev)
{
...
#if defined(FORCE_LOOPBACK)
        mfr |= SDR0_MFR_PKT_REJ_EN | SDR0_MFR_PKT_REJ_POL;
#endif
	mtsdr(sdr_mfr, mfr);
#endif
	...
	mfsdr (sdr_mfr, mfr);
	debug("enet: misc function register is 0x%08x\n", mfr);
#if !defined(FORCE_LOOPBACK)
	mfr &= ~SDR0_MFR_ETH_CLK_SEL_V(hw_p->devnum);
	mtsdr(sdr_mfr, mfr);
#endif
...
int ppc_4xx_eth_setup_bridge(int devnum, bd_t *bis)
{
	...
	case 0:
		...
		bis->bi_phymode[1] = BI_PHYMODE_NONE;
                ...
                value = RGMII_FER_GMII;
		/* 1 x GMII port */
		out32 (ZMII_FER, 0x00);
		out32 (RGMII_FER, value);
		break;
	case 1:
		/* 2 x RGMII ports */
		...
		bis->bi_phymode[0] = BI_PHYMODE_RGMII;
                value = RGMII_FER_RGMII;

#if defined(CONFIG_PHY1_ADDR)
		bis->bi_phymode[1] = BI_PHYMODE_RGMII;
                value |= RGMII_FER_RGMII << 4;
#endif
		out32 (ZMII_FER, 0x00);
                debug("enet: writing 0x%08x to RGMII FER\n", value);
		out32 (RGMII_FER, value);
	...
	/* Ensure we setup mdio for this devnum and ONLY this devnum */
	value = in32(ZMII_FER);
	value |= (ZMII_FER_MDI) << ZMII_FER_V(devnum);
	out32 (ZMII_FER, value);
        debug("enet: writing 0x%08x to ZMII FER\n", value);
...
static int ppc_4xx_eth_init (struct eth_device *dev, bd_t * bis)
{
	 ...
	 */
	printf ("enet%d: last transfer: tx=%d, rx=%d, handled=%d\n", hw_p->devnum, hw_p->stats.pkts_tx, hw_p->stats.pkts_rx, hw_p->stats.pkts_handled);
	...
	reg = 0;
	out32 (ZMII_FER, 0);
        debug("enet%d: clearing ZMII FER\n", devnum);
	...
	out32 (ZMII_SSR, ZMII_SSR_SP << ZMII_SSR_V(devnum));
        debug("enet%d: set ZMII SSR to 0x%08x\n", devnum, ZMII_SSR_SP << ZMII_SSR_V(devnum));
...
#if defined(FORCE_LOOPBACK)
        mode_reg |= EMAC_M1_ILE;
        debug("emac: enabling internal loopback mode (m1=0x%08x)\n", mode_reg);
#endif
...
#if defined(CONFIG_DP83865_PHY)
        miiphy_read (dev->name, reg, 0x12, &reg_short);
        reg_short = (reg_short & 0x0fff) | 0xb000;
        miiphy_write (dev->name, reg, 0x12, reg_short);
        debug("dp83865: extended settings 0x%04x\n", (unsigned int)reg_short);
#endif

#ifdef INFO_4XX_ENET
        printf(" MII PHY #%d transceiver registers:", reg);
        for(i = 0; i < 32; i++){
          miiphy_read (dev->name, reg, i, &reg_short);
          printf("%s%04x", (i % 8) ? " " : "\n   ", reg_short);
        }
        printf("\n");
#endif
...
#endif /* #if*N*def CONFIG_CS8952_PHY */
	...
	duplex = miiphy_duplex (dev->name, reg);
        ...
        speed = 1000;
        duplex = FULL;
        debug("enet: forcing 1Gbe fullduplex\n");
#endif

	if (hw_p->print_speed) {
		hw_p->print_speed = 0;
		printf ("enet%d: speed is %d Mbps - %s duplex connection\n", hw_p->devnum,
			(int) speed, (duplex == HALF) ? "HALF" : "FULL");

#if defined(CONFIG_DP83865_PHY)
                miiphy_read (dev->name, reg, 0x11, &reg_short);
                printf("enet: mdix=%s, txfifo=%s, speed=%d, link=%s\n", (reg_short & 0x0400) ? "crossover" : "straight", (reg_short & 0x0200) ? "error" : "ok", (((reg_short >> 4) & 0x1) * 99 + 1) * (((reg_short >> 3) & 0x1) * 9 + 1) * 10, (reg_short & 0x4) ? "good" : "absent");
#endif
	}
	...
	if ( (speed == 100) || (speed == 1000) )
        ...
        debug("enet%d: setting ZMII SSR to 0x%08x\n", hw_p->devnum, reg);
	out32 (ZMII_SSR, reg);
	...
	if ((devnum == 2) || (devnum == 3)) {
		if (speed == 1000)
			...
			return -1;
                ...
                debug("enet: setting RGMII SSR to 0x%08x\n", reg);
		out32 (RGMII_SSR, reg);
	}
#endif /* defined(CONFIG_440) && !defined(CONFIG_440SP) */
	...
	if (speed == 1000)
        ...
        debug("enet%d: setting RGMII SSR to 0x%08x\n", hw_p->devnum, reg);
	out32 (RGMII_SSR, reg);
#endif
    ...
    defined(CONFIG_440SP) || defined(CONFIG_440SPE)
#if 0
	mtdcr (malmcr, MAL_CR_PLBB | MAL_CR_OPBBL | MAL_CR_LEA | MAL_CR_PLBLT_DEFAULT | MAL_CR_EOPIE | 0x00330000);
#endif
        /* try without burst transfers */
	mtdcr (malmcr, MAL_CR_OPBBL | MAL_CR_LEA | MAL_CR_PLBLT_DEFAULT | MAL_CR_EOPIE | 0x00330000);
#else
	...
	for (i = 0; i < NUM_TX_BUFF; i++) {
		...
		hw_p->tx_run[i] = -1;
		debug("enet: TX_BUFF %d @ 0x%08lx\n", i, (ulong) hw_p->tx[i].data_ptr);
	}
	...
	out32 (EMAC_RXM + hw_p->hw_addr, EMAC_RMR_ARRP | EMAC_RMR_ARP | EMAC_RMR_AROP | EMAC_RMR_PME | EMAC_RMR_PMME | EMAC_RMR_BAE | EMAC_RMR_IAE);
#if 0
...
static int ppc_4xx_eth_send (struct eth_device *dev, volatile void *ptr, int len)
{
	...
	unsigned long temp_txm0, txst, rej, rxst;
	...
	out32 (EMAC_TXM0 + hw_p->hw_addr,
        ...
        switch(hw_p->devnum){
          case 0 :
            mfsdr (sdr_emac0rejcnt, rej);
            mfsdr (sdr_emac0rxst, rxst);
            break;
#if defined(CONFIG_HAS_ETH1)
          case 1 :
            mfsdr (sdr_emac1rejcnt, rej);
            mfsdr (sdr_emac1rxst, rxst);
            break;
#endif
          default :
            printf("enet: unknown device %d\n", hw_p->devnum);
            rej = rxst = 0;
            break;
        }
        debug("enet%d: tx: sending=%db, pkts sofar: tx=%d, rx=%d(+%dr) rxst=0x%08x\n", hw_p->devnum, len, hw_p->stats.pkts_tx, hw_p->stats.pkts_rx, rej, rxst);
        debug("enet%d: mr0=0x%08x, mr1=0x%08x, txm0=0x%08x, rxm=0x%08x\n", hw_p->devnum, in32(EMAC_M0 + hw_p->hw_addr), in32(EMAC_M1 + hw_p->hw_addr), in32(EMAC_TXM0 + hw_p->hw_addr), in32(EMAC_RXM + hw_p->hw_addr));
	hw_p->stats.pkts_tx++;
#endif
...
#ifdef INFO_4XX_ENET
                                switch(hw_p->devnum){
                                  case 0 : mfsdr (sdr_emac0txst, txst); break;
#if defined(CONFIG_HAS_ETH1)
                                  case 1 : mfsdr (sdr_emac1txst, txst); break;
#endif
                                  default : txst = 0; break;
                                }
                                debug("enet%d: tx fail, status=0x%08x\n", hw_p->devnum, txst);
#endif
				return (-1);
			}
		} else {
                        ...
                        debug("enet%d: tx ok, status=0x%08x\n", hw_p->devnum, txst);
#endif
			return len;
		}
	}
...
#ifdef INFO_4XX_ENET
                debug("enet%d: rx: got=%d bytes, packets sofar: tx=%d, rx=%d\n", hw_p->devnum, length, hw_p->stats.pkts_tx, hw_p->stats.pkts_rx);
...
#if defined(CONFIG_NET_MULTI)
                debug("enet: NET_MULTI configured\n");
#if defined(CONFIG_MII) || (CONFIG_COMMANDS & CFG_CMD_MII)
                debug("enet: registering mii for %s\n", dev->name);
