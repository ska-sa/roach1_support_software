 ...
 * Device code
 */
#ifndef CONFIG_DTT_I2C_DEV_CODE  
#define CONFIG_DTT_I2C_DEV_CODE 0x48 /* ON Semi's LM75 device */
#endif
...
int dtt_read(int sensor, int reg)
{
     ...
     */
    sensor = CONFIG_DTT_I2C_DEV_CODE + (sensor & 0x07); /* calculate address of lm75 */
...
int dtt_init (void)
{
	...
	if (_dtt_init(sensors[i]) != 0)
	    printf("%s%d (0x%02x) FAILED INIT\n", header, i+1, CONFIG_DTT_I2C_DEV_CODE);
	else
	    printf("%s%d is %i C\n", header, i+1,
