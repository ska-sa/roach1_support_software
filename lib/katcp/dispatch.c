...
int version_katcp(struct katcp_dispatch *d, char *subsystem, int major, int minor)
{
  ...
  s->s_version_major = major;
  s->s_version_minor = minor;

  if(subsystem){
    s->s_version_subsystem = strdup(subsystem);
    if(s->s_version_subsystem == NULL){
      return -1;
    }
  }
...
int build_katcp(struct katcp_dispatch *d, char *build)
{
  ...
  if(build){
    s->s_build_state = strdup(build);
    if(s->s_build_state == NULL){
      return -1;
    }
  }
...
void on_connect_katcp(struct katcp_dispatch *d)
{
  ...
  if(s->s_version_subsystem){
    ...
    append_args_katcp(d, KATCP_FLAG_LAST, "%s-%d.%d", s->s_version_subsystem, s->s_version_major, s->s_version_minor);
  }

  if(s->s_build_state){
    append_string_katcp(d, KATCP_FLAG_FIRST, "#build-state");
    append_args_katcp(d, KATCP_FLAG_LAST, "%s", s->s_build_state);
  }
}
...
{
  return register_mode_katcp(d, match, help, call, 0);
}
...
int lookup_katcp(struct katcp_dispatch *d)
{
  ...
  for(search = d->d_shared->s_commands; search; search = search->c_next){
    if(((search->c_mode == 0) || (search->c_mode == d->d_shared->s_mode)) && (!strcmp(search->c_name, s))){
#ifdef DEBUG
...
int log_message_katcp(struct katcp_dispatch *d, unsigned int priority, char *name, char *fmt, ...)
{
  ...
  fixed_name = name ? name : s->s_version_subsystem;
