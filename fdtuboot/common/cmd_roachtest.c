...
static int test_selectmap(void)
{
  if (usb_setup()){
    ...
    return -1;
  } else if (selectmap_program(SCRATCHPAD_A, V5LX110_BITFILE_SIZE, 0) && selectmap_program(SCRATCHPAD_A, V5LX110_BITFILE_SIZE, 1)) {
...
{
  const unsigned int offset = CONFIG_SYS_FPGA_BASE + (which ? IADC1_OFFSET: IADC0_OFFSET); 
  //stop the ADC
